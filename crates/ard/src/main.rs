...
fn run_skill(cmd: SkillCmd) -> i32 {
    match cmd.subcommand {
        ...
        SkillSubcommand::Install {
            ...
            let selected = if all || skill.is_empty() {
            ...
            if target.is_empty() {
                return fail("No --target specified (use --target codex and/or --target claude).");
            }

            for name in &selected {
                if !embedded_has_skill(name) {
                    return fail(&format!("Unknown embedded skill: {name}"));
                }
            }

            let mut targets: Vec<(SkillTarget, PathBuf)> = Vec::new();
            for t in target {
                let h = home.clone().unwrap_or_else(|| default_tool_home(t));
                targets.push((t, h));
            }

            for (t, home_dir) in targets {
                let tool = match t {
                    SkillTarget::Codex => "codex",
                    SkillTarget::Claude => "claude",
                };
                for name in &selected {
                    let dest = if let Some(ns) = &namespace {
                        home_dir.join("skills").join(ns).join(name)
                    } else {
                        home_dir.join("skills").join(name)
                    };

                    if dry_run {
                        println!("[DRY-RUN] {name} -> {tool}:{dest}", dest = dest.display());
                        continue;
                    }
                    ...
                    if let Err(err) = write_embedded_skill_to(&name, &dest) {
                        return fail(&format!("Failed to install {name}: {err}"));
                    }
                    println!("[OK] Installed {name} -> {tool}:{dest}", dest = dest.display());
                }
            }
...
fn run_lint(cmd: LintCmd) -> i32 {
    match cmd.subcommand {
        ...
        Some(LintSubcommand::Skill { skill_dir, max_lines }) => {
        ...
        Some(LintSubcommand::Docset {
            ...
            let (errors, warnings) =
                lint_docset(&root, &docs_dir, &examples_dir, check_doc_placeholders, cmd.strict);
            emit_lint_result(
                LintResult {
                    path: None,
...
fn lint_auto(path: PathBuf, strict: bool, format: OutputFormat, root_override: Option<PathBuf>) -> i32 {
    let p = canonicalish(&path);
    if p.is_file() {
        let file_name = p.file_name().and_then(|s| s.to_str()).unwrap_or("");
        if file_name == "AGENTS.md" {
            let (errors, warnings) = lint_agents_md(&p, strict);
            return emit_lint_result(
                LintResult {
        ...
        if p.extension().and_then(OsStr::to_str).unwrap_or("") == "md" {
            let root = match root_override {
                Some(r) => r,
                None => match infer_docset_root(p.parent().unwrap_or(Path::new("."))) {
                    Some(r) => r,
                    None => {
                        return fail(
                            "Could not infer docset root for file; pass --root <dir> or lint a directory.",
                        )
                    }
                },
            };
        ...
        return fail("Unsupported file type for auto linting.");
    }

    if p.is_dir() {
        if p.join("SKILL.md").is_file() {
            let (errors, warnings) = lint_skill(&p, 500);
            return emit_lint_result(
                LintResult {
                    path: Some(p.to_string_lossy().to_string()),
                    root: None,
        ...
        let (errors, warnings) = lint_docset(&p, "docs", "examples", false, strict);
        return emit_lint_result(
            LintResult {
                path: None,
                root: Some(p.to_string_lossy().to_string()),
                pass: errors.is_empty(),
                error_count: errors.len(),
                warning_count: warnings.len(),
                errors,
    ...
    fail(&format!("Path does not exist: {}", p.display()))
}
...
const VAGUE_PHRASES: [&str; 5] = ["as appropriate", "follow conventions", "use standard", "when necessary", "see docs"];
...
fn lint_agents_md(path: &Path, strict: bool) -> (Vec<Finding>, Vec<Finding>) {
    ...
    if !critical.is_empty() {
        let never_topic_patterns: BTreeMap<&str, Regex> = BTreeMap::from([
            ("force push", Regex::new(r"(?im)\bNEVER\b.*\bforce\s+push\b").unwrap()),
            ("hook", Regex::new(r"(?im)\bNEVER\b.*\bhook(?:s)?\b").unwrap()),
            ("secret", Regex::new(r"(?im)\bNEVER\b.*\bsecret(?:s)?\b").unwrap()),
            ("generated", Regex::new(r"(?im)\bNEVER\b.*\bgenerated\b").unwrap()),
        ]);
...
const ALLOWED_FRONTMATTER_KEYS: [&str; 6] = ["name", "description", "license", "compatibility", "metadata", "allowed-tools"];
...
fn parse_frontmatter(frontmatter_text: &str) -> Result<Frontmatter, String> {
    ...
    while index < lines.len() {
        ...
        let rest = caps.get(2).map(|m| m.as_str()).unwrap_or("").trim_end().to_string();
...
fn is_probably_markdown(path: &Path) -> bool {
    matches!(
        path.extension().and_then(OsStr::to_str).unwrap_or("").to_lowercase().as_str(),
        "md" | "markdown"
    )
}
...
fn lint_skill(skill_dir: &Path, max_lines: usize) -> (Vec<Finding>, Vec<Finding>) {
    ...
    if openai_yaml.exists() {
        if let Ok(contents) = fs::read_to_string(&openai_yaml) {
            let required = ["interface:", "display_name:", "short_description:", "default_prompt:"];
            let missing: Vec<&str> = required.iter().copied().filter(|k| !contents.contains(k)).collect();
            if !missing.is_empty() {
                errors.push(Finding {
                    check_id: "SK013".to_string(),
                    severity: "error".to_string(),
                    message: format!("agents/openai.yaml missing required keys: {}", missing.join(", ")),
...
fn parse_yaml_subset(frontmatter_block: &str) -> Result<BTreeMap<String, YamlValue>, String> {
    ...
    for raw_line in frontmatter_block.lines() {
        ...
        if rest.is_empty() {
            // Start nested map
            set_in_map(&mut root, &parent_path, &key, YamlValue::Map(BTreeMap::new()))?;
...
fn set_in_map(
    ...
    for p in &parent_path[1..] {
        match current {
            YamlValue::Map(map) => {
                current = map.entry(p.clone()).or_insert_with(|| YamlValue::Map(BTreeMap::new()));
            }
...
fn push_to_list(
    ...
    for (idx, p) in key_path[1..].iter().enumerate() {
        ...
        match current {
            ...
            YamlValue::List(_) => return Err("Invalid YAML subset: list nested under list.".to_string()),
...
fn parse_markdown_table(section_text: &str) -> (Vec<String>, Vec<Vec<String>>) {
    let lines: Vec<&str> = section_text.lines().map(|l| l.trim_end_matches('\n')).collect();
    ...
    for (pattern, token) in EXAMPLE_PLACEHOLDER_PATTERNS {
        ...
        for m in re.find_iter(text) {
            ...
            let finding = Finding {
                ...
                evidence: format!("{}: {}", path.display(), line.trim().chars().take(200).collect::<String>()),
            };
...
fn is_decisionful_uc(text: &str) -> bool {
    ...
    let contract_keywords = [
        "schema", "contract", "breaking", "deprecat", "compat", "migration", "version", "protobuf", "openapi",
    ];
...
fn lint_docset(
    ...
    for uc in &uc_docs {
        ...
        let Some(glossary_path) = resolve_local_path(uc.path.parent().unwrap_or(&root), glossary_link) else {
            continue;
        };
        if !glossary_path.exists() {
            continue;
        }
        let Some(glossary_doc) = read_doc(&glossary_path) else { continue };
        ...
        let Some(uc_ent_col) = uc_ent_col else { continue };
    ...
    for doc in &docs {
        let Some(cap) = h1_re.captures(&doc.text) else { continue };
    ...
    if examples_path.is_dir() {
        let has_uc_example = docs.iter().any(|d| d.path.starts_with(&examples_path) && d.doc_type.as_deref() == Some("use_case"));
        let has_glossary_example = docs.iter().any(|d| d.path.starts_with(&examples_path) && d.doc_type.as_deref() == Some("glossary"));
        let has_nfr_example = docs.iter().any(|d| d.path.starts_with(&examples_path) && d.doc_type.as_deref() == Some("nfr"));
        ...
        let requires_nfr_example = docs.iter().any(|d| {
            d.path.starts_with(&examples_path)
                ...
                && d.risk_tier
                    .as_ref()
                    .map(|t| matches!(t.trim().to_lowercase().as_str(), "tier1" | "tier2" | "tier3"))
                    .unwrap_or(false)
        });
        if requires_nfr_example && !has_nfr_example {
            warnings.push(Finding {
                ...
                severity: "warning".to_string(),
                message: "No NFR baseline example found under examples/ (recommended for tier1+).".to_string(),
                evidence: examples_path.display().to_string(),
            });
        }
    }
    ...
    for adr in &adr_docs {
        ...
        for v in use_cases {
            if let YamlValue::Str(s) = v {
                let uc_id = s.trim();
                if !uc_id.is_empty() {
                    adr_uc_links.entry(uc_id.to_string()).or_default().push(adr.path.clone());
                }
            }
        }
    }
...
mod tests {
    ...
    fn docset_linter_detects_duplicate_ids() {
        ...
        assert!(errors.iter().any(|f| f.check_id == "DS-ID-002"), "{errors:?}");
    }
    ...
    fn docset_linter_requires_adr_for_decisionful_tier2_uc() {
        ...
        let (errors, _warnings) = lint_docset(tmp.path(), "docs", "examples", false, false);
        assert!(errors.iter().any(|f| f.check_id == "DS-CF-004"), "{errors:?}");
    }
}
