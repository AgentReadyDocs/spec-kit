...
description: >
  Authors (generates or updates) AGENTS.md — a token-efficient context file for AI coding agents.
...
The rubric is owned by: `$reviewing-agents-md` → `reviewing-agents-md/references/agents-md-rubric.md` (single source of truth).
...
- [Analysis Phase](#analysis-phase) — Steps 0-4: gather facts from the project
- [AGENTS.md Template](#agentsmd-template) — exact output structure (16 sections)
- [When to Re-run](#when-to-re-run) — triggers for regeneration
- [User Review](#user-review) — approval flow before writing
- [Execution](#execution) — write AGENTS.md + CLAUDE.md, merge strategy
- [Validation](#validation) — 15-point post-write checklist (structural + quality)
- [Quality Gate](#quality-gate) — single-pass review for ambiguity, contradictions, missing guards
- [Output Rules](#output-rules) — prose style, safety, idempotency
...
**File & env safety:**
- NEVER read `.env`, `*credentials*`, `*secret*`, `*.pem`, `*.key` files or any file matched by `.gitignore` secret patterns
...
**When NOT to generate:** Do not generate for: single-file scripts, throwaway PoCs with no build config, or repos with only a README. Inform user the project lacks enough structure. Minimum: a build config file with at least one runnable command.
...
**CLAUDE.md content** — always includes the generated header `<!-- Generated by authoring-agents-md. Custom edits (extra @imports, overrides) are preserved on re-run. -->` plus imports:
...
15. **Rubric self-check** — grade the file using `reviewing-agents-md/references/agents-md-rubric.md`. If score < 4.5/5.0, fix P1/P2 gaps (CRITICAL, Commands, Structure, accuracy) and re-validate.
...
Run a **fresh-context critic** pass using `$reviewing-agents-md` on the generated `AGENTS.md`:
- If your environment supports subagents, **spawn a fresh-context subagent** to run the review. Otherwise, invoke `$reviewing-agents-md` directly.
- Apply **P1 + P2** fixes (P3 last).
- Re-run Validation checks.
- Repeat up to **2 loops**, stop early when the Quality Bar is met.

Single-pass review after validation. Read the completed AGENTS.md as if encountering the project for the first time. Do NOT loop — run once, fix what's fixable, flag the rest.
...
- Do not re-score or re-grade. Do not iterate.
...
3. **Safety** — do NOT execute commands that modify data or infrastructure
4. **No web by default** — do not browse the web or call external systems unless the user explicitly requests it
