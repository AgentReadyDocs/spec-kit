...
description: >
  Reviews and grades an `AGENTS.md` file for completeness, clarity, accuracy, and agent effectiveness.
  Produces a structured assessment with dimension scores, prioritized findings, and concrete recommendations.
  Use when: the user asks to review, audit, grade, evaluate, or score an `AGENTS.md` (or similar agent context file).
...
Evaluate an AGENTS.md file from the perspective of an AI coding agent encountering the project for the first time. Produce a structured quality report with scores, prioritized findings, and actionable recommendations. **This skill is read-only** — it reports problems but does not modify files.

## When NOT to use

- Do not use to generate or update `AGENTS.md` (use `$authoring-agents-md`).
- Do not use when the target artifact is not an agent context file; ask for the correct path/artifact first.

## Contents

- [Input](#input) — what to read and verify
- [Review Dimensions](#review-dimensions) — 7 weighted scoring dimensions
- [Scoring](#scoring) — grade calculation
- [Output Format](#output-format) — exact report structure
- [Review Guidelines](#review-guidelines) — judgment criteria
- [Output Rules](#output-rules) — style and constraints
...
2. Otherwise, look for `AGENTS.md` in CWD. If not found and you're in a git repo, also check the repo root (e.g., `git rev-parse --show-toplevel`).
3. If not found, check `CLAUDE.md` (or similar agent-instruction files) for an `AGENTS.md` reference/import (e.g., `@AGENTS.md`), and follow the referenced path.
4. If no file found, inform the user and stop.
...
1. **Report only** — do not modify `AGENTS.md` or any project files. If the user wants fixes, recommend they re-run `authoring-agents-md` with targeted updates or apply changes manually.
...
See `references/example-review.md` for a realistic example report.
